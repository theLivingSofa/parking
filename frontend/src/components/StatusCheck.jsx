...
function StatusCheck() {
  ...
  const handleScan = async (result) => {
    if (result) {
      setLoading(true);
      toast.loading('Fetching user status...', { id: 'statusFetch' });

      try {
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/status`, {
          qrcodeUrl: result.text,
        });

        setStatus(response.data);
        setError('');
        toast.success('Status fetched successfully!', { id: 'statusFetch' });
      } catch (err) {
        console.error(err);
        setError(err.response?.data?.message || 'An error occurred');
        toast.error(err.response?.data?.message || 'Failed to fetch status', { id: 'statusFetch' });
        setStatus(null);
      } finally {
        setLoading(false);
        setScanning(false);
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="space-y-6">
        ...
        {loading && (
          <div className="text-center text-indigo-600 text-sm font-medium">Loading...</div>
        )}
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-md">
            ...
            <dl className="mt-2 space-y-2">
              ...
              <div>
                ...
                <dd className="text-sm text-gray-900">
                  {status.currentCheckIn ? new Date(status.currentCheckIn).toLocaleString() : 'Not checked in'}
                </dd>
              </div>
              <div>
                <dt className="text-sm font-medium text-gray-500">Status</dt>
